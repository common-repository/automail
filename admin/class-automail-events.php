...
class Automail_Events
{
        ...
        if ( function_exists( 'get_userdata' ) and function_exists( 'get_user_meta' ) ) {
            ...
            if ( $user_id ) {
                ...
                $this->automail_log(
                    ...
                    __METHOD__,
                    "101",
                    "ERROR: wordpress_newUser fired but no User ID . " . $user_id
                );
            }
            ...
            $this->automail_log(
                ...
                __METHOD__,
                "102",
                "ERROR: get_userdata or get_user_meta is not Exist"
            );
        }
        ...
        if ( function_exists( 'get_userdata' ) && function_exists( 'get_user_meta' ) && !empty($user_id) ) {
            ...
            if ( $user_id && $user->ID ) {
                ...
                $this->automail_log(
                    ...
                    __METHOD__,
                    "103",
                    "ERROR: wordpress_UserProfileUpdate fired but no User ID . " . $user_id
                );
            }
        ...
        if ( function_exists( 'get_userdata' ) && function_exists( 'get_user_meta' ) && !empty($user_id) ) {
            ...
            if ( $user_id && $user->ID ) {
                ...
                $this->automail_log(
                    ...
                    __METHOD__,
                    "105",
                    "ERROR: wordpress_deleteUser fired but no User ID . " . $user_id
                );
            }
        ...
        if ( function_exists( 'wp_get_current_user' ) && !empty(wp_get_current_user()->ID) ) {
            ...
            if ( $user->ID ) {
                ...
                $this->automail_log(
                    ...
                    __METHOD__,
                    "108",
                    "ERROR:  wordpress_userLogout fired but no User ID . " . $user->ID
                );
            }
            ...
            if ( !in_array( $post->post_status, array( 'auto-draft', 'draft', 'trash' ) ) ) {
                ...
                if ( $post_time_diff < 5 ) {
                    ...
                    $this->automail_log(
                        ...
                        __METHOD__,
                        "200",
                        "SUCCESS: testing the post edited publish. " . $post_id
                    );
                } else {
    ...
    public function automail_wpforms_process( $fields, $entry, $form_data )
    {
        $this->automail_log(
            ...
            __METHOD__,
            "111",
            "TESTING: fields : " . json_encode( $fields )
        );
        $this->automail_log(
            get_class( $this ),
            __METHOD__,
            "112",
            "TESTING: entry : " . json_encode( $entry )
        );
        $this->automail_log(
            get_class( $this ),
            __METHOD__,
            "113",
            "TESTING: form_data :" . json_encode( $form_data )
        );
        # if There is a integration on this Form Submission
     ...
     * @since    1.0.0
     */
    public function automail_weforms_entry_submission(
        $entry_id,
        $form_id,
        $page_id,
        $form_settings
    )
    {
        ...
        if ( !empty($form_id) and !empty($entry_id) ) {
            ...
            $this->automail_log(
                ...
                __METHOD__,
                "125",
                "ERROR: weform form_id or entry_id is empty."
            );
        }
        ...
        if ( isset( $form_id ) ) {
            ...
            if ( !empty($dataArray) and !empty($form_id) ) {
                ...
                $this->automail_log(
                    ...
                    __METHOD__,
                    "127",
                    "ERROR: forminator Form entries are empty Or form_id is empty!"
                );
            }
    ...
    public function automail_send_mail( $eventDataSourceID = "", $eventDataArray = "" )
    {
        ...
        if ( empty($eventDataSourceID) or !is_string( $eventDataSourceID ) ) {
            $this->automail_log(
                ...
                __METHOD__,
                "128",
                "ERROR: eventDataSourceID  is empty or not string."
            );
        ...
        if ( empty($eventDataArray) or !is_array( $eventDataArray ) ) {
            $this->automail_log(
                ...
                __METHOD__,
                "129",
                "ERROR: eventDataArray is empty or not array."
            );
        ...
        if ( empty($emailAutomatons) ) {
            $this->automail_log(
                ...
                __METHOD__,
                "130",
                "WARNING: no mail automation saved in the dataBase. Create a Email Automaton."
            );
            ...
            if ( isset( $emailAutomaton->post_excerpt ) and $emailAutomaton->post_excerpt == $eventDataSourceID ) {
                ...
                if ( is_array( $mailReceiver ) and !empty($mailReceiver) ) {
                    ...
                    if ( $this->automail_userRoles()[0] ) {
                        ...
                        // **** there should be an error when there is no UserRole; so handel it if not empty()
                        $roles = array();
                        ...
                        $this->automail_log(
                            ...
                            __METHOD__,
                            "131",
                            "ERROR: User role is empty or False automail_userRoles() func is not working!"
                        );
                    }
                    ...
                    if ( $automailThisLastFired and time() - $automailThisLastFired < 25 ) {
                        ...
                        $this->automail_log(
                            ...
                            __METHOD__,
                            "132",
                            "WARNING: Repeated submission Prevented : <b>" . $emailAutomaton->ID . "</b> " . $emailAutomaton->post_title
                        );
                    ...
                    $this->automail_log(
                        ...
                        __METHOD__,
                        "134",
                        "ERROR:  Empty email receiver : Check this automation meta : " . json_encode( array( $eventDataSourceID, $eventDataArray ) )
                    );
    ...
    public function automail_userRoles()
    {
        ...
        if ( $wp_user_roles ) {
            ...
            $this->automail_log(
                ...
                __METHOD__,
                "135",
                "ERROR: wp_user_roles OPTION is empty on option Table."
            );
        }
        ...
        if ( !empty($userRoles) ) {
            ...
            $this->automail_log(
                ...
                __METHOD__,
                "137",
                'ERROR: $userRoles variable is empty in automail_userRoles() function.'
            );
